...
async function bootstrap(): Promise<void> {
    ...
    app.useGlobalFilters(new AllExceptionHttpFilter(createLogger('ExceptionFilter'), HttpException, InternalServerErrorException));

    if (!IS_PROD || IS_ATTACH_SWAGGER) attachSwagger(app, MAIN_LANG, DOCS_PATH, IS_SECURE);

    await app.listen(PORT, () => logger.info(`Gateway-operator for ${DOMAIN} started on ${PORT}`));
}
